...
**发布状态：实验版，尚未提交官方市场。** 已验证 GitHub 安装、真实 Codex 钩子发现、离线/HTTP/进程/文件系统测试。真实 Codex 的手动/自动压缩与立即续跑已用本地模拟模型/Jev 验证；真实 Jev API 已验证；三种场景真实模型小样本对照双方均答对，尚未证明优于原生；官方市场暂缓提交。[测试报告](docs/TEST-REPORT.md) · [提交草案](docs/SUBMISSION.md)。
...
`plugins/codex-jev-compaction/.env.example` 仅为配置说明，插件不会自动读取 `.env`。从桌面图标启动的进程不一定继承终端环境。需要 Codex 构建支持文档中的压缩钩子；已实测本机 CLI 0.154.0 的压缩钩子链路，但不将其宣称为最低兼容版本。

关闭：移除 `JEV_ENABLE=1` 或设置为 `0`，重启对应 Codex 进程；也可以在 Codex 中禁用插件。

## 工作方式

1. 只处理配对成功、结果为字符串或纯文本块数组的 `function_call` / `custom_tool_call`。未知类型、非文本结果、未完成调用保留在导出副本中。
...
5. 保留结果时也保留调用；只保留调用时将导出结果截断至前 300 字符；否则成对移除。**当评分预览不完整时，无论分数高低都保留完整记录**，防止丢失模型没看到的中间事实。这类记录的决策理由为 `partial-preview`；不承诺裁剪长日志。文本消息不会在导出副本中被改写。
